    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (req.session && req.query.state) {
          const redirectProperty = redirectPropertyName(req.query.state)
        ...
        return passport.authenticate(loginSilentStrategy, { state: newState, successRedirect: nextUrl })(req, res, next)
      })
    } else {
