...
class OIDC {
  ...
  verifyBasicConfiguration(basicConfig) {
    ...
    keys.forEach((key) => {
      if (basicConfig[key] === undefined) {
        throw new Error(
          `OpenID Connect setup: Missing configuration for ${key}`
        );
      }
    });
  }
   ...
   *
   * @todo Secure cookie?
   * @todo failureRedirect, how should this work?
   *
   ...
   * @param {string} [config.appCallbackSilentLoginUrl] Optional The silent callback URL used for setting up the express route. Same as config.callbackUrl without host. Example: /node/auth/silent/callback
   * @param {string} [config.callbackLogoutUrl] Optional This apps full URL to callback function for logout. Example: http://localhost:3000/node/auth/silent/callback
   * @param {string} [config.appCallbackLogoutUrl] Optional The silent callback URL used for setting up the express route. Same as config.callbackUrl without host. Example: /node/auth/logout/callback
   ...
   * @param {function} [config.extendUser] Optional Function which gives you the possibility to add custom properties to the user object. Example: (user, claims) => { user.isAwesome = true }
   */
  ...
  constructor(
    ...
    const loginIsConfigured = this.isCallbackConfigured(
      "login",
      callbackLoginUrl,
      appCallbackLoginUrl
    );
    ...
    if (!loginIsConfigured) {
      throw new Error(
        "Mandatory OIDC configuration for login. Bad configuration?"
      );
    }
    ...
    if (
       ...
       * @param {Object} res - Express response object
       *
       * @returns {Promise<Middleware>} A promise which resolves to a middleware which logs the current user
       *
       * @example oidc.logout
       */
     ...
     *
     * @returns {Middleware} Promise which resolves to a Express middleware
     *
