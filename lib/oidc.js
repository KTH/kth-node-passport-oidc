    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          ...
          // const noToolbarCookieFound = !this.foundToolbarCookie(req)
          const noToolbarCookieFound = true

          if (req.session.anonymous && noToolbarCookieFound) {
          ...
          // On multiple simultaneous silent requests we need to hold of to many requests. Each request coming to this point
          // will get a unique cookie.
          ...
          if (true) {
            ...
            res.redirect(`/auth/silent/bounce?nextUrl=${req.url}&requestTime=${currentTimeInMilliseconds}`)
          } else {
            return next()
          }
        })(req, res, next)
