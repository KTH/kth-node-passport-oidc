...
class OIDC {
  ...
  constructor(
    ...
    OIDC.prototype.logCall = (action, log, req, state, nextUrl) => {
      try {
        log.debug(
          `${action} ${commonLogInfo(req)} nextUrl: ${nextUrl} state: ${state} user: ${req.user && req.user.username}`
        )
      } catch (error) {
        // Let it go...
      }
    ...
    OIDC.prototype.throwError = (req, info) => {
      throw new Error(`${info} ${commonLogInfo(req)}`)
    }
    ...
    const loginStrategy = new Promise(async (resolve, reject) => {
      ...
      const loginStrategy = new Strategy(
        ...
        (req, tokenSet, done) => {
          ...
          createUserFromClaims(tokenSet.claims(), extendUser).then(user => {
    ...
    OIDC.prototype.login = async function (req, res, next) {
      ...
      return ((req, res, next) => {
        thiz.log.debug(`kth-node-passport-oidc: Login ${commonLogInfo(req)}`)
        if (typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
          thiz.log.debug(
            `kth-node-passport-oidc: User was already logged in when arriving at Silent Login. Forwarding with next() ${commonLogInfo(
              req
            )}`
          )
          return next()
        ...
        if (req.method === 'HEAD') {
          thiz.log.debug(`kth-node-passport-oidc: Login does not support method HEAD ${commonLogInfo(req)}`)
        ...
        thiz.logCall('kth-node-passport-oidc: Login', log, req, newState, req.url)
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      ...
      if (req.method === 'HEAD') {
        thiz.log.debug(
          `kth-node-passport-oidc: Login callback: Login does not support method HEAD ${commonLogInfo(req)}`
        )
        return res.status(400).send('Login does not support method HEAD')
      }

      thiz.logCall('kth-node-passport-oidc: Login callback', log, req, state, nextUrl)
    ...
    if (isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          if (req.method === 'HEAD') {
            thiz.log.debug(
              `kth-node-passport-oidc: SilentLogin: Login does not support method HEAD ${commonLogInfo(req)}`
            )
          ...
          if (typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
            thiz.log.debug(
              `kth-node-passport-oidc: SilentLogin: User was logged in when arriving at Silent Login. Forwarding with next() ${commonLogInfo(
                req
              )}`
            )
            return next()
          ...
          thiz.log.debug(
            `kth-node-passport-oidc: SilentLogin: toolbarCookieFound: ${toolbarCookieFound} and numberOfTries: ${numberOfTries} ${commonLogInfo(
              req
            )}`
          )
          ...
          if (!toolbarCookieFound) {
            thiz.log.debug(
              `kth-node-passport-oidc: SilentLogin: no toolbarCookieFound, but found these cookies: ${JSON.stringify(
                req.cookies
              )} ${commonLogInfo(req)}`
            )
          }
          ...
          if (toolbarCookieFound && numberOfTries < 3) {
            ...
            thiz.logCall('kth-node-passport-oidc: SilentLogin', log, req, newState, req.url)
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (nextUrl && typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
          thiz.logCall(
            'kth-node-passport-oidc: SilentLogin callback. Was already authenticated',
            log,
            req,
            state,
            nextUrl
        ...
        if (req.query.error) {
          if (
            ...
            thiz.logCall('kth-node-passport-oidc: SilentLogin callback. Is anonymous', log, req, state, nextUrl)
            return res.redirect(nextUrl)
          } else {
        ...
        thiz.logCall('kth-node-passport-oidc: SilentLogin callback', log, req, state, nextUrl)
        ...
        thiz.logCall(
          'kth-node-passport-oidc: Expected authorization request details in session is missing',
          log,
          req,
          state,
          nextUrl
        ...
        log.debug(
          `Authorization session info: state: ${state} redirectpropertyname: ${redirectPropertyName(state)}`,
          `Session info : ${JSON.stringify(req.session[state])}`,
          `Stored nexturl in session: ${req.session[redirectPropertyName(state)]}`
        )
        if (req && req.session && req.session[redirectPropertyName(state)]) {
          return passport.authenticate(
            createSilentLoginStrategy(client, callbackSilentLoginUrl, redirectPropertyName(state)),
            {
              state,
              successRedirect: nextUrl,
            }
        ...
        thiz.logCall(
          'kth-node-passport-oidc: Expected authorization request details in session is missing, after second attempt',
          log,
          req,
          state,
          nextUrl
        ...
        log.debug(
          `kth-node-passport-oidc: Authorization session info: state: ${state} redirectpropertyname: ${redirectPropertyName(
            state
          )}`,
          `Session info : ${JSON.stringify(req.session[state])}`,
          `Stored nexturl in session: ${req.session[redirectPropertyName(state)]}`
