...
class OIDC {
  ...
  logEvent(req, info) {
    try {
      log.debug(`${info} sessionID: ${req.sessionID && req.sessionID.substring(0, req.sessionID.length - 5)} `)
    } catch (error) {
      // Let it go...
    }
  }
  ...
  constructor(
    ...
    OIDC.prototype.login = async function (req, res, next) {
      ...
      return ((req, res, next) => {
        if (typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
          logEvent(req, 'User was already logged in when arriving at Silent Login. Forwarding with next()')
          return next()
        }
        ...
        req.session[redirectPropertyName(state)] = req.url

        logCall('Login', log, req, state, req.url)
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      ...
      logCall('Login callback', log, req, state, req.url)
      ...
      if (!nextUrl) {
        throw new Error('Missing nextUrl. Current session is not the one that started the login')
      }
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          if (req.session.anonymous || (typeof req.isAuthenticated === 'function' && req.isAuthenticated())) {
            logEvent(req, 'User was already logged in/anonymous when arriving at Silent Login. Forwarding with next()')
            return next()
          }
          ...
          logCall('SilentLogin', log, req, state, req.url)
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (req.session.anonymous || (typeof req.isAuthenticated === 'function' && req.isAuthenticated())) {
          logCall('SilentLogin callback. Was already authenticated', log, req, state, req.url)
          return res.redirect(nextUrl)
        }
        ...
        if (req.query.error) {
          if (
            ...
            logCall('SilentLogin callback. Is anonymous', log, req, state, req.url)
        ...
        logCall('SilentLogin callback', log, req, state, req.url)
