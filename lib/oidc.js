  ...
  constructor(
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        // If no nextUrl is found then it is most likely stored in a session on another instance
        // We redirect and hope it will end up on the right instance this time.
        if (!nextUrl) {
          setTimeout(() => res.redirect(req.originalUrl), 1000)
        }
      ...
      expressApp.get(callbackSilentLoginBounceRoute, this.silentLogin, async (req, res, next) => {
        ...
        return res.redirect(`${nextUrl}?bounced=true`)
      })
    } else {
