  ...
  constructor(
    ...
    OIDC.prototype.login = async function (req, res, next) {
      ...
      return ((req, res, next) => {
        ...
        req.session[newState] = req.url

        try {
          log.debug(
            `Login sessionID: ${req.sessionID && req.sessionID.substring(0, req.sessionID.length - 5)} req.url: ${
              req.url
            } req.originalUrl: ${req.originalUrl} state: ${newState} redirectForState: ${req.session[newState]}`
          )
        } catch (error) {
          // Let it go...
        }
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      const nextUrl = req.session[req.query.state] || defaultRedirect
      delete req.session[req.query.state]
      ...
      try {
        ...
        log.debug(
          ...
          ]} redirects: ${req.session && req.session.redirects && JSON.stringify(req.session.redirects)}`
        )
      } catch (error) {
        // Let it be
      }
