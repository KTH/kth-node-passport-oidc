...
class OIDC {
  ...
  constructor(
    ...
    OIDC.prototype.login = async function login(req, res, next) {
      ...
      return ((req, res, next) => {
        log.debug(`kth-node-passport-oidc: Login ${commonLogInfo(req)}`)
    ...
    if (isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function silentLogin(req, res, next) {
        ...
        return ((req, res, next) => {
          if (req.method === 'HEAD') {
            log.debug(`kth-node-passport-oidc: SilentLogin: Login does not support method HEAD ${commonLogInfo(req)}`)
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (req.query.error) {
          if (
            ...
            log.debug(`kth-node-passport-oidc: SilentLogin callback. Is anonymous', log, req, state, nextUrl)
