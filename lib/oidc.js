...
class OIDC {
   ...
   * @param {number} config.anonymousThreshold Time in milliseconds before a anonymous session should try a login again?
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          ...
          // timeToCheck - on the first ever request, 'anonymous' has not been set, or the time since 'anonymous' was set
          // exceeds the time in anonymousThreshold. This is for handling multiple requests, to let the user be anonymous
          // for a short period of time, but long enough to handle multiple requests (i.e. 20 images)
          const timeToCheck =
            req.session.anonymous === undefined || new Date().getTime() - req.session.anonymous > anonymousThreshold

          thiz.logEvent(req, `SilentLogin: toolbarCookieFound: ${toolbarCookieFound} and timeToCheck: ${timeToCheck}`)

          if (!toolbarCookieFound) {
            thiz.logEvent(req, `SilentLogin: no toolbarCookieFound, but found these cookies: ${req.cookies}`)
          }

          if (toolbarCookieFound && timeToCheck) {
            const newState = state()
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (req.query.error) {
          if (
            ...
            // State and time so we know that a login has been tried, but failed
            req.session.anonymous = new Date().getTime()
      ...
      /**
       * Setup of express route. This route is used to bounce clients against the app to ensure that a cookie exist
       * before using the app.
       */

      if (callbackSilentLoginBounceRoute) {
        expressApp.get(callbackSilentLoginBounceRoute, this.silentLogin, async (req, res, next) => {
          if (req.method === 'HEAD') {
            thiz.logEvent(req, 'SilentLogin callback: Login does not support method HEAD')
            return res.status(400).send('Silent login callback does not support method HEAD')
          }

          // Store a dummy value to ensure that a session is created
