  ...
  constructor(
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      ...
      if (!nextUrl) {
        thiz.throwError(
          req,
          `Login callback: Missing nextUrl. Current session is not the one that started the login state ${state} method ${req.method}`
        )
      }
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          if (req.session.anonymous || (typeof req.isAuthenticated === 'function' && req.isAuthenticated())) {
            thiz.logEvent(
              req,
              'SilentLogin: User was already logged in/anonymous when arriving at Silent Login. Forwarding with next()'
            )
            return next()
          }
