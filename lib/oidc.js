...
class OIDC {
  ...
  constructor(
     ...
     *
     */
    OIDC.prototype.login = async function login(req, res, next) {
      ...
      return ((req, res, next) => {
        ...
        thiz.log.debug(
          `kth-node-passport-oidc: Login ${commonLogInfo(req)} nextUrl: ${req.url} state: ${newState} user: ${
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      ...
      thiz.log.debug(
        `kth-node-passport-oidc: Login callback ${commonLogInfo(req)} nextUrl: ${nextUrl} state: ${state} user: ${
          req.user && req.user.username
        }`
      )
      if (!state) {
    ...
    if (isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
       ...
       *
       */
      OIDC.prototype.silentLogin = async function silentLogin(req, res, next) {
        // eslint-disable-next-line no-unused-vars
        const client = await oidcClient
        return ((req, res, next) => {
          if (req.method === 'HEAD') {
          ...
          if (toolbarCookieFound && numberOfTries < 3) {
            ...
            req.session[redirectPropertyName(newState)] = req.url
            thiz.log.debug(
              `kth-node-passport-oidc: SilentLogin ${commonLogInfo(req)} nextUrl: ${req.url} state: ${newState} user: ${
                req.user && req.user.username
              }`
            )
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (nextUrl && typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
          thiz.log.debug(
            `kth-node-passport-oidc: SilentLogin callback. Was already authenticated ${commonLogInfo(
              req
        ...
        if (req.query.error) {
          if (
            ...
            thiz.log.debug(`kth-node-passport-oidc: SilentLogin callback. Is anonymous', log, req, state, nextUrl)
              ${commonLogInfo(req)} nextUrl: ${nextUrl} state: ${state} user: ${req.user && req.user.username}`)
            return res.redirect(nextUrl)
          }
        ...
        thiz.log.debug(
          `kth-node-passport-oidc: SilentLogin callback ${commonLogInfo(
            req
          )} nextUrl: ${nextUrl} state: ${state} user: ${req.user && req.user.username}`
        )
        ...
        thiz.log.debug(
          `kth-node-passport-oidc: Expected authorization request details in session is missing ${commonLogInfo(
            req
          )} nextUrl: ${nextUrl} state: ${state} user: ${
            req.user && req.user.username
