...
async function createUserFromClaims(claims, extendUser = async () => {}) {
  ...
  const memberOf = []
  if (typeof claims.memberOf === 'string') memberOf.push(claims.memberOf)
  else if (Array.isArray(claims.memberOf)) memberOf.push(...claims.memberOf)
...
class OIDC {
   ...
   * @param {boolean} [config.checkIsOwner] Optional flag with false as default. When used with requireRole, req.param.username is checked if it has the same username as logged in user.
   */
  ...
  requireRole =
    ...
    (req, res, next) => {
      const user = req.user || {}
      if (this.checkIsOwner && roles.includes('isOwner')) {
        const isOwner =
          user.username && req.params && req.params.username ? req.params.username === user.username : false
        if (isOwner) {
          return next()
        }
      }
