  ...
  constructor(
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
          ...
          try {
            log.debug(
              `SilentLogin sessionID: ${
                ...
                req.session.redirects[newState]
              }`
            )
          } catch (error) {
            // Let it go...
      ...
      expressApp.get(callbackSilentLoginRoute, (req, res, next) => {
        ...
        if (req.session && req.query.state && req.session[req.query.state]) {
          nextUrl = req.session[req.query.state]
          delete req.session[req.query.state]
        }
        ...
        // is no need to authenticate again. Let the user through.
        if (req.session.anonymous || (typeof req.isAuthenticated === 'function' && req.isAuthenticated())) {
          return res.redirect(nextUrl)
        }
