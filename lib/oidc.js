...
class OIDC {
  ...
  isCurrentRequestTheFirstRequest(requestTime, appCookieName, cookies) {
    const oldestTimeInCookies = Object.entries(cookies)
      .map(([key, value]) => {
        var regex = new RegExp(`\\d*-${appCookieName}`, 'g')
        if (key.match(regex)) {
          return value
        }
      })
    ...
    return requestTime == oldestTimeInCookies
  }

  // silentLoginCookies
  //   .filter((cookieName) = cookieName. !== undefined))
  //   .map(cookieName => {
  //     return { time: cookieName.match('/([0-9]+)-.'), cookie: cookieName }
  //   })
  ...
  logCall(action, log, req, state, nextUrl) {
    try {
      log.debug(
   ...
   * @param {string} [config.callbackSilentLoginBounceRoute] Optional The silent callback bounce to handle multiple request. Example: /node/auth/silent/bounce
   ...
   * @param {string} config.sessionKey Session key for the app i.e. the name of the default app cookie
   * @param {boolean} config.secureCookie Should silent cookie handling use secure cookies?
  ...
  constructor(
    ...
    passport,
    {
      ...
      callbackSilentLoginBounceRoute,
      callbackLogoutUrl,
      callbackLogoutRoute,
      defaultRedirect,
      sessionKey,
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          ...
          if (req.session.anonymous && !toolbarCookieFound) {
            thiz.logEvent(req, 'SilentLogin: User was anonymous when arriving at Silent Login. Forwarding with next()')
            return next()
          }

          // Arriving here means that we initiate silent login.

          // On multiple simultaneous silent requests we need to hold of to many requests. If there are more
          // than one silentLoginCookie existing at this point we redirect with a timeout
          const silentLoginCookies = Object.keys(req.cookies).filter(cookieName => cookieName.includes(sessionKey))

          thiz.logEvent(req, 'SilentLogin: silentLoginCookies: ' + JSON.stringify(silentLoginCookies))

          if (silentLoginCookies.length > 1) {
            thiz.logEvent(req, 'SilentLogin: not first call, redirecting with timeout')
            return setTimeout(() => res.redirect(req.url), 1000)
          }

          // Each request coming to this point will get a unique cookie.
          if (toolbarCookieFound) {
            const currentTimeInMilliseconds = new Date().getTime()

            res.cookie(`${currentTimeInMilliseconds}-${sessionKey}`, currentTimeInMilliseconds, {
              maxAge: 5000, // TODO - should this be configurable?
              secure: secureCookie,
              // httpOnly Sätt den här!
            })

            thiz.logCall('SilentLogin', log, req, undefined, req.url)

            res.redirect(
              `${callbackSilentLoginBounceRoute}?nextUrl=${req.url}&requestTime=${currentTimeInMilliseconds}`
            )
          } else {
            return next()
          }
      ...
      /**
       * Setup of express route. This route is used to bounce the request for the silent login.
       * By bouncing the incoming requests we assure that we get all cookies from the client/browser.
       *
       * Each request to this route passes a timestamp and the original url. We need to pick one
       * request to start the app session because otherwise the different requests overwrite each other.
       *
       * The request with the oldest timestamp (the first one) is chosen to start the session, and the
       * rest are held back thorough a timeout
       */
      expressApp.get(callbackSilentLoginBounceRoute, async (req, res, next) => {
        const client = await oidcClient
        if (req.method === 'HEAD') {
        ...
        // On multiple simultaneous calls an earlier call could already have authenticated the user.
        // Then there is no need to authenticate again.
        ...
        if (req.cookies.length === 0) {
          thiz.throwError(req, 'SilentLogin bounce: No cookies found. The client does not support cookies?')
        }
        ...
        if (!requestTime) {
          thiz.logEvent(req, `SilentLogin bounce: Missing requestTime param`)
          return res.status(400).send('Missing requestTime param')
        }

        if (thiz.isCurrentRequestTheFirstRequest(requestTime, sessionKey, req.cookies)) {
          const newState = state()

          req.session[redirectPropertyName(newState)] = nextUrl

          thiz.logCall('SilentLogin', log, req, newState, nextUrl)

          return passport.authenticate(createSilentLoginStrategy(client, callbackSilentLoginUrl, newState), {
            state: newState,
          })(req, res, next)
        } else {
          return setTimeout(() => res.redirect(nextUrl), 1000)
        }
      })
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (req.query.error) {
          if (
            ...
            // State so we know that a login has been tried, but failed
