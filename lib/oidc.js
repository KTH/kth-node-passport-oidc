...
class OIDC {
  ...
  constructor(
    ...
    OIDC.prototype.logEvent = (req, info) => {
      try {
        log.debug(`${info} ${commonLogInfo(req)}`)
      } catch (error) {
        // Let it go...
      }
    }
    ...
    if (isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (!state) {
          thiz.logEvent(req, 'SilentLogin callback: Missing required parameter: state')
        ...
        thiz.logCall('Expected authorization request details in session is missing', log, req, state, nextUrl)
        log.debug(
        ...
        thiz.logCall(
          'Expected authorization request details in session is missing, after second attempt',
          log,
          req,
          state,
          nextUrl
