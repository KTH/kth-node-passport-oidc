  ...
  constructor(
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      try {
          ...
          thiz.logCall('Expected authorization request details in session is missing', log, req, state, nextUrl, req.session[state], req.session[redirectPropertyName(state)])
          return next()
        }
      })
    } else {
