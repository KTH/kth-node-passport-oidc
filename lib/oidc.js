...
class OIDC {
  ...
  constructor(
    ...
    OIDC.prototype.login = async function (req, res, next) {
      ...
      return ((req, res, next) => {
        if (typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
          this.logEvent(req, 'User was already logged in when arriving at Silent Login. Forwarding with next()')
          return next()
        }
        ...
        this.logCall('Login', log, req, newState, req.url)
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      ...
      this.logCall('Login callback', log, req, state, req.url)
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          ...
          this.logCall('SilentLogin', log, req, newState, req.url)
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (req.session.anonymous || (typeof req.isAuthenticated === 'function' && req.isAuthenticated())) {
          this.logCall('SilentLogin callback. Was already authenticated', log, req, state, req.url)
          return res.redirect(nextUrl)
        }
        ...
        if (req.query.error) {
          if (
            ...
            this.logCall('SilentLogin callback. Is anonymous', log, req, state, req.url)
        ...
        this.logCall('SilentLogin callback', log, req, state, req.url)
