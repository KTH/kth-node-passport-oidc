...
class OIDC {
  ...
  constructor(
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      ...
      thiz.logCall('Login callback', log, req, state, req.url)
      ...
      if (!nextUrl) {
        throw new Error(`callbackLoginRoute: Missing nextUrl. Current session is not the one that started the login state ${state} method ${req.method}`)
      }
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          if (req.session.anonymous || (typeof req.isAuthenticated === 'function' && req.isAuthenticated())) {
            thiz.logEvent(
              req,
              'User was already logged in/anonymous when arriving at Silent Login. Forwarding with next()'
            )
            return next()
          }
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (!nextUrl) {
          throw new Error(`callbackSilentLoginRoute: Missing nextUrl. Current session is not the one that started the login. state: ${state} method: ${req.method}`)
        }
        ...
        if (req.session.anonymous || (typeof req.isAuthenticated === 'function' && req.isAuthenticated())) {
          thiz.logCall('SilentLogin callback. Was already authenticated', log, req, state, req.url)
          return res.redirect(nextUrl)
        }
        ...
        if (req.query.error) {
          if (
            ...
            thiz.logCall('SilentLogin callback. Is anonymous', log, req, state, req.url)
        ...
        thiz.logCall('SilentLogin callback', log, req, state, req.url)
