  ...
  constructor(
    ...
    OIDC.prototype.login = async function (req, res, next) {
      ...
      return ((req, res, next) => {
        ...
        try {
          log.debug(
            `Login sessionID: ${req.sessionID && req.sessionID.substring(0, req.sessionID.length - 5)} req.url: ${
              req.url
            } req.originalUrl: ${req.originalUrl} state: ${newState} redirectForState: ${
              req.session[redirectPropertyName(newState)]
            }`
          )
        } catch (error) {
          // Let it go...
    ...
    expressApp.get(callbackLoginRoute, (req, res, next) => {
      // TODO handle non existing state
      const nextUrl = req.session[redirectPropertyName(req.query.state)] || defaultRedirect

      try {
        const username = () =>
          req.user ? req.user.username : req.session && req.session.anonymous ? req.session.anonymous : 'unknown'
        log.debug(
          `Login Callback. sessionID: ${req.sessionID && req.sessionID.substring(0, req.sessionID.length - 5)} State: ${
            req.query.state
          } Next URL: ${nextUrl} HasSession: ${!!req.session} User.username: ${username()} ARD: ${!!req.session[
            'oidc:login.ug.kth.se'
          ]} redirects: ${req.session && req.query && req.query.state && req.session[req.query.state]}`
        )
      } catch (error) {
        // Let it be
      }

      delete req.session[redirectPropertyName(req.query.state)]
    ...
    if (this.isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function (req, res, next) {
        ...
        return ((req, res, next) => {
          ...
          try {
            log.debug(
              `SilentLogin sessionID: ${
                req.sessionID && req.sessionID.substring(0, req.sessionID.length - 5)
              } req.url: ${req.url} req.originalUrl: ${req.originalUrl} state: ${newState} redirectForState: ${
                req.session[redirectPropertyName(newState)]
              }`
            )
          } catch (error) {
          ...
          return passport.authenticate(loginSilentStrategy, { state: newState })(req, res, next)
        })(req, res, next)
      }
      ...
      expressApp.get(callbackSilentLoginRoute, async (req, res, next) => {
        ...
        if (req.session && state) {
          const redirectProperty = redirectPropertyName(state)
          nextUrl = req.session[redirectProperty]
          delete req.session[redirectProperty]
        }
        ...
        try {
          const username = () =>
            req.user ? req.user.username : req.session && req.session.anonymous ? req.session.anonymous : 'unknown'
          log.debug(
            `SilentLogin Callback. sessionID: ${
              req.sessionID && req.sessionID.substring(0, req.sessionID.length - 5)
            } State: ${state} Next URL: ${nextUrl} HasSession: ${!!req.session} User.username: ${username()} ARD: ${!!req
              .session[state]}`
          )
        } catch (error) {
          // Let it be
        ...
        return passport.authenticate(loginSilentStrategy, { state, successRedirect: nextUrl })(req, res, next)
      })
    } else {
