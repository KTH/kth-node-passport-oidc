...
class OIDC {
    ...
    OIDC.prototype.login = async function login(req, res, next) {
      ...
      return ((req, res, next) => {
        ...
        if (typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
          log.debug(`kth-node-passport-oidc: User was already logged in when arriving at Silent Login.`)
          return next()
        }
    ...
    if (isCallbackConfigured('silent', callbackSilentLoginUrl, callbackSilentLoginRoute)) {
      ...
      OIDC.prototype.silentLogin = async function silentLogin(req, res, next) {
        ...
        return ((req, res, next) => {
          ...
          if (typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
            log.debug(`kth-node-passport-oidc: SilentLogin: User was logged in when arriving at Silent Login.`)
            return next()
          }
          ...
          log.debug(
            `kth-node-passport-oidc: SilentLogin: toolbarCookieFound: ${toolbarCookieFound}, numberOfTries: ${numberOfTries} ${commonLogInfo(
              req
            )}`
          )
  ...
  logout = async (req, res) => {
    ...
    if (req.session['_id_token']) {
      const id_token_hint = decrypt(req.session['_id_token'], this.tokenSecret)
      const client = await this.oidcClient
