...
test("extractDisplayName should return the same value if called with a string or with an array", () => {
  expect( 
    OIDC.prototype.extractDisplayName(["Name"])
  ).toBe("Name");
...
test("should throw error if no callback provided", () => {
  expect(() =>
    OIDC.prototype.isCallbackConfigured("login", () => {}, null)
  ).toThrow();
});
...
test("it creates user from claims", () => {
  const claims = { ugUsername: "jonsnow", unique_name: ["Jon Snow"], email: "jonsnow@kth.se", memberOf: "utveckling"}
  return OIDC.prototype.createUserFromClaims(claims).then(data => {
    expect(data).toEqual({"displayName": "Jon Snow", "email": "jonsnow@kth.se", "memberOf": "utveckling", "username": "jonsnow"})
  })
});
